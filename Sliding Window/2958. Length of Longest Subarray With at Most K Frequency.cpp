...
class Solution { // Same Q. is -: Longest substring with atmost k distinct char.s 
public:
    int maxSubarrayLength(vector<int>& nums, int k) {
        ...
        while(j<n){
            mp[nums[j]]++; // <ele,freq>
            while(mp[nums[j]]>k){ // mp[s[j]] means freq of the curr ele. 
                mp[nums[i]]--;
                if(mp[nums[i]]==0){ // ele'freq is 0 then
                    mp.erase(nums[i]); // delete those ele. from the mp
                }
                i++;
            }
            // update after each iterations -: 
            len=j-i+1;
