...
where the ith battery can run a computer for batteries[i] minutes. You are interested in running 
...
public:
    long long maxRunTime(int n, vector<int>& batteries) {
        ...
        long long mini=*min_element(batteries.begin(), batteries.end());
        long long maxi=0;
        for(int i=0; i<m; i++){
            maxi+=batteries[i]; 
        }

        // Perform Binary Search On Ans -: 
        while(mini<maxi){
            long long mid=mini+(maxi-mini+1)/2;
            long long sum=0;
            for(int i=0; i<m; i++){
